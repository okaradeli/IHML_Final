...
def get_sales_dataset():
    ...
    if glb.EXPERIMENT_FEATURES:
        X=X[glb.EXPERIMENTAL_FEATURE_LIST_FOREST_20]
...
def get_higgs_boson_dataset():
    ...
    print("Higgs Boson data loading...")
    X = pd.read_csv('data/higgs/training.csv')
    logger.info("Raw data len:" + str(len(X)))
    print("Raw data len:" + str(len(X)))
    X = X.rename(columns=lambda x: re.sub('[^A-Za-z0-9_]+', '', x))##This stupid line is required for LGBM classifier
    if glb.DATASET_DO_SAMPLE:
        X = X.sample(glb.DATASET_SAMPLE_SIZE, random_state=42)
        logger.info("Sampled  data len:" + str(len(X))+" column size:"+str(len(X.columns)))
        print("Sampled  data len:" + str(len(X)) + " column size:" + str(len(X.columns)))
    ...
    if glb.EXPERIMENT_FEATURES:
        ...
        logger.info("Reduced Set: " + str(glb.EXPERIMENTAL_FEATURE_LIST_FOREST_20))
        X=X[glb.EXPERIMENTAL_FEATURE_LIST_FOREST_20]
...
def get_magic_dataset():
    ...
    logger.info("MAGIC DATA Column count:"+str(len(X.columns)))
    logger.info("MAGIC DATA Target Value count:" + str(np.unique(Y)))
...
def get_forest_dataset():
    ...
    logger.info("FOREST DATA Column count:"+str(len(X.columns)))
    logger.info("FOREST DATA Target Value count:" + str(np.unique(Y)))
