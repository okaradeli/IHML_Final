...
def get_top_features(model,alpha, X, Y,dataset):
        ...
        try:
            ...
            if glb.EXPERIMENT_FEATURES:
                for feature in features:
                    if feature not in glb.EXPERIMENTAL_FEATURE_LIST_FOREST_20:
    ...
    if(glb.DATASET=="qsar"):
        feature_names = [col for col in X_test.columns if col != 'result']
        # 2. Shuffle the feature names randomly
        random.shuffle(feature_names)
        # 3. Generate random values (e.g., uniform between 0 and 1)
        random_scores = np.random.rand(len(feature_names))


        # Build the DataFrame with custom column names
        shuffled_df = pd.DataFrame({
            'col_name': feature_names,
            'feature_importance_vals': random_scores
        })

        # 5. Sort by score descending
        feature_importance = shuffled_df.sort_values(by='feature_importance_vals', ascending=False).reset_index(drop=True)
