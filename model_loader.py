...
def get_stacking(models,base_model_extended=False):
    ...
    #level1 = XGBClassifier(random_state=13)
...
def evaluate_model_train(model, X, Y):
    ...
    if glb.DATASET in ["baddebt","sales"] and (glb.SCORER in [f1_score,recall_score,precision_score,roc_auc_score]):##this dataset is MULTI-CLASS giving issues of Y values like: ValueError: Invalid classes inferred from unique values of y.  Expected: [0 1 2 3 4 5 6], got [1 2 3 4 5 6 7]  any idea ?
        scorer = make_scorer(glb.SCORER, average='weighted')
...
def train_evaluate_incremental_model(meta_learner_list, feature_list, X, Y):
    ##X = X[feature_list]
    valid_features = [c for c in feature_list if c in X.columns] ##For scalability tests
    X = X[valid_features]
    ...
    #return np.max(scores) ##test


# create a base learner and evaluate its score
def train_incremental_model(meta_learner_list, feature_list, X,Y):
    #X = X[feature_list]
    valid_features = [c for c in feature_list if c in X.columns] ##for scability tests
    X = X[valid_features]
